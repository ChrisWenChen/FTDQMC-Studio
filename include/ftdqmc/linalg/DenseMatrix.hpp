...
class DenseMatrix {
 public:
  DenseMatrix() = default;
  DenseMatrix(int rows, int cols) : rows_(rows), cols_(cols), data_(static_cast<size_t>(rows) * cols) {}
 ...
 private:
  int rows_ = 0;
  int cols_ = 0;
  std::vector<Complex> data_; // column-major: index = i + rows * j

  size_t index(int i, int j) const {
    return static_cast<size_t>(i) + static_cast<size_t>(rows_) * static_cast<size_t>(j);
  }
};
